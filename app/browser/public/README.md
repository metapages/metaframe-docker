...
3. Run a worker (or a bunch):
   ```
   docker run --restart unless-stopped -tid -v /var/run/docker.sock:/var/run/docker.sock -v /tmp:/tmp ghcr.io/metapages/metaframe-docker-worker:0.0.13 --cpus=2 --queue=public1
   ```
